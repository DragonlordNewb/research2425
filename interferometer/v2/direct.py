...
if __name__ == "__main__":
	...
	if len(sys.argv) == 1:
		...
		for i in range(8):
			try:
				print("  Scanning COM" + str(i) + + " ...", end="")
				sys.stdout.flush()
				ser = serial.Serial("COM" + str(i), BAUD_RATE, timeout=1)
				ser.close()
				print("Located live port.")
				com = i
				break
			except:
				print("Port is dead.")
		if com == -1:
			print("Error: Could not find live port.")
			exit(-1)
		print("Connection good on COM" + str(com) + ".")
		port = "COM" + str(com)
	else:
		port = sys.argv[1]

	root = tk.Tk()
	app = InterferometerClientApp(root, port)
	root.mainloop()